...
async function checkMissions(userId) {
  ...
  try {
    ...
    try {
      ...
      const dailyId = `daily_${todayStr}`;
      
      // Look up if user has an active daily mission generated by Gamemaster
      const dailyResult = await client.query(
        `SELECT mission_def FROM user_missions WHERE user_id = $1 AND mission_id = $2`,
        [userId, dailyId]
      );

      if (dailyResult.rows.length > 0 && dailyResult.rows[0].mission_def) {
        const dailyDef = dailyResult.rows[0].mission_def;
        
        // Puzzles are launched for tomorrow, so the vote_date recorded in DB is tomorrow's date!
