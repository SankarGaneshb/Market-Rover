...
def generate_report_body(issues):
    ...
    if not issues:
        return f"No open issues found for {date_str}."

    report = [f"# Daily Issue Report - {date_str}\n"]
    report.append("### Open Issues Summary\n")
    ...
    if count == 0:
        return f"No open issues (excluding PRs) found for {date_str}."

    report.append(f"\n**Total Open Issues:** {count}")
    report.append("\n*Generated by Market Rover Daily Report Bot*")
...
def main():
    print("Starting Daily Issue Report generation...")
    ...
    # 4. Create New Issue (Only if there are issues, or maybe always?)
    # Validating if we should create an issue if empty. The user generally wants a report.
    # We'll create it.
    
    today = datetime.datetime.now().strftime("%Y-%m-%d")
    issue_title = f"Daily Issue Report: {today}"
    
    # To prevent spam, we can check if an issue with this title already exists for today
